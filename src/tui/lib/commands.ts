...
const COMMANDS: CommandDefinition[] = [
  { key: 'Type', action: 'type filter text', section: 'Filter', footer: true, when: anyMode('query') },
  ...
  { key: 'j/k', action: 'next/prev match', section: 'Filter', footer: true, when: anyMode('filtered-nav') },
  { key: '/', action: 'edit filter', section: 'Filter', footer: true, when: anyMode('filtered-nav') },
  { key: 'Esc', action: 'clear filter', section: 'Filter', footer: true, when: anyMode('filtered-nav') },
  { key: 'Enter', action: 'inspect selected worktree', section: 'Worktrees', footer: true, when: paneMode('worktrees', 'normal') },
  { key: 'o', action: 'open selected worktree', section: 'Worktrees', footer: true, when: paneMode('worktrees', 'normal') },
  { key: '/', action: 'filter worktrees', section: 'Worktrees', footer: true, when: paneMode('worktrees', 'normal') },
  { key: 'u', action: 'bring selected services up', section: 'Worktrees', footer: true, when: paneMode('worktrees', 'normal') },
  { key: 'd', action: 'bring selected services down', section: 'Worktrees', footer: true, when: paneMode('worktrees', 'normal') },
  { key: 'a', action: 'archive selected worktree', section: 'Worktrees', footer: true, when: paneMode('worktrees', 'normal') },
  { key: 'r', action: 'refresh', section: 'Worktrees', footer: true, when: paneMode('worktrees', 'normal') },
  { key: 'Enter', action: 'open selected service', section: 'Services', footer: true, when: paneMode('services', 'normal') },
  { key: '/', action: 'filter services', section: 'Services', footer: true, when: paneMode('services', 'normal') },
  { key: 'd', action: 'bring worktree down', section: 'Services', footer: true, when: paneMode('services', 'normal') },
  { key: 'x', action: 'kill host service', section: 'Services', footer: true, when: paneMode('services', 'normal') },
  { key: 'Esc', action: 'cancel', section: 'Confirm', footer: true, when: state => Boolean(state.pendingAction) },
  { key: 'y', action: 'confirm', section: 'Confirm', footer: true, when: state => Boolean(state.pendingAction) },
  { key: 'n', action: 'cancel', section: 'Confirm', footer: true, when: state => Boolean(state.pendingAction) },
  { key: 'Type', action: 'search commands', section: 'Palette', footer: true, when: state => state.palette.open },
  { key: 'Enter', action: 'run', section: 'Palette', footer: true, when: state => state.palette.open },
  { key: 'Esc', action: 'close palette', section: 'Palette', footer: true, when: state => state.palette.open },
...
export function getHelpSectionsForState(state: TuiInteractionState): HelpSection[] {
  ...
  const focusSection = state.pendingAction
    ...
    : state.palette.open
      ? 'Palette'
      : state.panes[state.activePane].mode === 'query' || state.panes[state.activePane].mode === 'filtered-nav'
        ? 'Filter'
