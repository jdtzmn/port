...
describe('onboard command', () => {
  ...
  test('prints onboarding flow with command, how, and why guidance', async () => {
    ...
    expect(mocks.header).toHaveBeenCalledWith('4. port shell-hook <bash|zsh|fish>')
    expect(mocks.header).toHaveBeenCalledWith('5. port enter <branch>')
    expect(mocks.header).toHaveBeenCalledWith('10. port open')
